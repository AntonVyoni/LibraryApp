...
public class DVDSearchResultController implements Initializable {
...
//	public String subjects;
	...
	public DVDSearchResultController() {
		
	}
	
	public DVDSearchResultController(String titles, String directorFirstName, String directorLastName, String countries, String genres) {
		this.titles = titles;
	...
	public void openDeleteTitle(ActionEvent event) {
		  ...
		  try {
	    	if(table.getSelectionModel().getSelectedItem()==null) {
				...
				DVDModel selectedMediaIDs = table.getSelectionModel().getSelectedItem();
				mediaIDs = selectedMediaIDs.getMediaID();
				DVDModel selectedBookIDs = table.getSelectionModel().getSelectedItem();
			    dvdIDs = selectedBookIDs.getDvdID();
			    DVDModel selectedBook = table.getSelectionModel().getSelectedItem();
		    	titles = selectedBook.getTitle();
				FXMLLoader loader = new FXMLLoader(getClass().getResource("/application/DeleteDVDTitle.fxml"));
				  DeleteDVDTitleController delDVDTitCon = new DeleteDVDTitleController(mediaIDs, dvdIDs, titles);
				  loader.setController(delDVDTitCon);
				  Parent root = loader.load();         
				  Stage stage = new Stage();
				  stage.setScene(new Scene(root));
				  stage.show();
...
//	  		  System.out.println(iID+" "+accID);
	...
	@FXML
	public void openEditDVD(ActionEvent event)  {
	  	  try {
	  		if(table.getSelectionModel().getSelectedItem()==null) {
				selectItemPrompt.setText("Välj ett föremål i listan");
			}else {
	  		...
	  		DVDModel selectedBook = table.getSelectionModel().getSelectedItem();
	    	titles = selectedBook.getTitle();
		    DVDModel selectedAuthorFirstName = table.getSelectionModel().getSelectedItem();
		    authorFirstNames = selectedAuthorFirstName.getDirectorFirstName();
		    DVDModel selectedAuthorLastName = table.getSelectionModel().getSelectedItem();
		    authorLastNames = selectedAuthorLastName.getDirectorLastName();
		    DVDModel selectedCountry = table.getSelectionModel().getSelectedItem();
		    countries = selectedCountry.getCountry();
		    DVDModel selectedSubject = table.getSelectionModel().getSelectedItem();
		    genres = selectedSubject.getGenre();
		    DVDModel selectedDescription = table.getSelectionModel().getSelectedItem();
		    descriptions = selectedDescription.getDescription();
		    DVDModel selectedMediaIDs = table.getSelectionModel().getSelectedItem();
		    mediaIDs = selectedMediaIDs.getMediaID();
		    DVDModel selectedBookIDs = table.getSelectionModel().getSelectedItem();
		    int dvdIDs = selectedBookIDs.getDvdID();
		    DVDModel selectedAuthorIDs = table.getSelectionModel().getSelectedItem();
		    authorIDs = selectedAuthorIDs.getAuthorID();
		    DVDModel selectedAgeRestriction = table.getSelectionModel().getSelectedItem();
		    ageLimit = selectedAgeRestriction.getAgeRestriction();
	  		  FXMLLoader loader = new FXMLLoader(getClass().getResource("/application/EditDVD.fxml"));
			  EditDVDController editDVDCon = new EditDVDController(titles, genres, authorFirstNames, authorLastNames, 
					  												releaseDate, descriptions, countries, dvdIDs, authorIDs, mediaIDs, ageLimit);
			  loader.setController(editDVDCon);
			  Parent root = loader.load();         
			  Stage stage = new Stage();
			  stage.setScene(new Scene(root));
			  stage.show();
